...
using System.Linq;
using cav2010.algorithms;
using cav2010.datastructure;
...
namespace PAT.Common.Classes.DataStructure
{
    ...
    // */
    //private Automata quotient(Automata fa, HashSet<Pair<FAState,FAState>> rel) {
    //    Automata result=new Automata(fa.InitialState.ID);
    ...
    //    foreach (FAState state in fa.States.Values)
    //    {
    //        map.Add(state, state);
    //        foreach (FAState state2 in fa.States.Values)
    //        {
    ...
    //    reducedMap.Add(map[fa.InitialState], result.InitialState);
	...
	private bool double_graph_test(Pair<Arc,HashSet<Arc>> g, Pair<Arc,HashSet<Arc>> h, string init){
		Arc g_arc=g.Left;
		Arc h_arc=h.Left;
	...
	private List<Pair<Arc, HashSet<Arc>>> buildSingleCharacterSuperGraphs(){
	    ...
	    foreach (string sym in system.alphabet)
	    {
	        HashSet<Arc> graph=new HashSet<Arc>();

	        foreach (FAState from in spec.States.Values)
	        {
	            if(from.Post.ContainsKey(sym)){

	                foreach (FAState to in from.Post[sym])
	                {
	...
	void computeSim(){
		...
		states=system.States.Values.ToArray();		

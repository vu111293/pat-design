...
using WndProcCallBack = Fireball.Docking.Helpers.PInvoke.WndProcCallBack;
...
namespace Fireball.Docking
{
	...
	internal class DragHandlerBase : IMessageFilter
	{
        ...
        private static IntPtr SetWindowLong(IntPtr hWnd, int nIndex, WndProcCallBack wndProcCallBack)
        {
            if (Win32Helper.IsWindowsOS())
            {
                return PInvoke.SetWindowLong(hWnd, nIndex, wndProcCallBack);
            }
            else
            {
                return IntPtr.Zero;
        ...
        private static IntPtr SetWindowLong(IntPtr hWnd, int nIndex, IntPtr wndFunc)
        {
            if (Win32Helper.IsWindowsOS())
            {
                return PInvoke.SetWindowLong(hWnd, nIndex, wndFunc);
            }
            else
            {
                return IntPtr.Zero;
        ...
        private static IntPtr CallWindowProc(IntPtr prevWndFunc, IntPtr hWnd, int iMsg, IntPtr wParam, IntPtr lParam)
        {
            if (Win32Helper.IsWindowsOS())
            {
                return PInvoke.CallWindowProc(prevWndFunc, hWnd, iMsg, wParam, lParam);
            }
            else
            {
                return IntPtr.Zero;

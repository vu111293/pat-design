...
namespace Fireball.Docking
{
	...
	public class DockPaneStripVS2005 : DockPaneStripBase
	{
		...
		private void DrawTab_Document(Graphics g, IDockableWindow content, Rectangle rect)
		{
			...
			if (DockPane.DockPanel.ShowDocumentIcon)
			{
				rectText.X += DocumentTextExtraWidth / 2 + DocumentIconWidth + 2; //+ DocumentIconGapLeft;
			...
			if (DockPane.ActiveContent == content)
			{
                ...
                LinearGradientBrush lnbr = new LinearGradientBrush(rect,this.ActiveBackColor2, this.ActiveBackColor1, LinearGradientMode.Vertical );
				...
				if (DockPane.DockPanel.ShowDocumentIcon)
				{
					...
					Rectangle rectIcon = new Rectangle(rect.X + DocumentIconGapLeft, rect.Y + (rect.Height - DocumentIconHeight) / 2, DocumentIconWidth, DocumentIconHeight);
			...
			else
			{
                ...
                LinearGradientBrush lnbr = new LinearGradientBrush(rect, this.ActiveBackColor1,this.ActiveBackColor2, LinearGradientMode.Vertical);
        ...
        private void ScrollLeft_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Tabs.Count; i++)
            {
                if (DockPane.ActiveContent == Tabs[i].Content)
                {
                    ...
                    DockPane.ActiveContent = Tabs[i].Content;
                    DockPane.Activate();
                    Update();
	    ...
	    private void ScrollRight_Click(object sender, EventArgs e)
		{
		    ...
		    for (int i = 0; i < Tabs.Count; i++)
		    {
                if (DockPane.ActiveContent == Tabs[i].Content)
                {
                    DockPane.ActiveContent = Tabs[(i + 1) % Tabs.Count].Content;
                    DockPane.Activate();
                    Update();

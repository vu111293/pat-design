...
namespace ICSharpCode.SharpDevelop.Gui
{
	public static class WorkbenchSingleton
	{
        ...
        public static DockContainer DockContainer;
		...
		public static Control ActiveControl {
            get
            {
                return DockContainer.ActiveDocument as Control;
            }
		}


        public static IViewContent GetControlByFileName(string fileName)
        {
            foreach (IDockableWindow document in WorkbenchSingleton.DockContainer.Documents)
            {
                if (document is ITextEditorControlProvider)
                {
                    if ((document as ITextEditorControlProvider).TextEditorControl.FileName == fileName)
                    {
                        return document as IViewContent;
                    }
                }
            }
        ...
        public static ITextEditorControlProvider GetTextEditorByFileName(string fileName)
        {
            foreach (IDockableWindow document in WorkbenchSingleton.DockContainer.Documents)
            {
                if (document is ITextEditorControlProvider)
                {
                    if ((document as ITextEditorControlProvider).TextEditorControl.FileName == fileName)
                    {
                        return document as ITextEditorControlProvider;
                    }
                }
            }
